...
float AScavBullet::Trace(FVector start, FVector PreviousVelocity, float delta, TEnumAsByte<ECollisionChannel> CollisionChannel) {
	...
	if (Hit) {
		...
		EScavPenTraceType PenTraceType = DefaultPenTraceType;
		...
		//impact actual
		if (HasAuthority()) {
			OnImpact(Ricochet, Penetration, HitResult.Location, Velocity, HitResult.Normal, GetActorLocation(), NewVelocity, Impulse, PenetrationDepth, HitResult.Actor.Get(), HitResult.Component.Get(), HitResult.BoneName, HitResult);
		}
...
TArray<AActor*> AScavBullet::GetAttachedActorsRecursive(AActor* Actor, uint16 Depth) const{
