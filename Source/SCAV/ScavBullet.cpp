...
// Called every frame
void AScavBullet::Tick(float DeltaTime) {
...
void AScavBullet::Step(float DeltaTime) {
...
inline float AScavBullet::GetCurveValue(const UCurveFloat* curve, float in, float deflt) const {
	if (curve == nullptr) return deflt;
	return curve->GetFloatValue(in);
}

void AScavBullet::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift) {
