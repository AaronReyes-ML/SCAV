...
#include "ScavBullet.generated.h"

UENUM(BlueprintType)
enum class EScavAtmosphereType : uint8
{
	AT_Constant1 UMETA(DisplayName = "Constant1"),
	AT_Curve1 UMETA(DisplayName = "Density Curve1"),
	AT_Earth1 UMETA(DisplayName = "Earth/IGL1")
};

UCLASS(Blueprintable, BlueprintType)
class SCAV_API AScavBullet : public AActor
{
	GENERATED_BODY()
...
public:	
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, SaveGame, Category = "World", meta = (ToolTip = "Select atmosphere model")) EScavAtmosphereType AtmosphereType;
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Impact") EScavPenTraceType DefaultPenTraceType = EScavPenTraceType::PT_BackTrace1;
	...
	UFUNCTION(BlueprintCallable, Category = "EBBullet|Spawn")
		static void SpawnWithExactVelocity(TSubclassOf<class AScavBullet> BulletClass, AActor* BulletOwner, APawn* BulletInstigator, FVector BulletLocation, FVector BulletVelocity);

	UFUNCTION(BlueprintCallable, Category = "EBBullet|Spawn")
		static void Spawn(TSubclassOf<class AScavBullet> BulletClass, AActor* BulletOwner, APawn* BulletInstigator, FVector BulletLocation, FVector BulletVelocity);
	...
	UFUNCTION(BlueprintAuthorityOnly, BlueprintNativeEvent, Category = "EBBullet|Impact")
		void OnImpact(bool Ricochet, bool PassedThrough, FVector Location, FVector IncomingVelocity, FVector Normal, FVector ExitLocation, FVector ExitVelocity, FVector Impulse, float PenetrationDepth, AActor* Actor, USceneComponent* Component, FName BoneName, FHitResult HitResult);
...
private:
	UPROPERTY() TArray<TWeakObjectPtr<AScavBullet>> Pooled;
	static AScavBullet* GetFromPool(UWorld* World, UClass* BulletClass);
	static AScavBullet* SpawnOrReactivate(UWorld* World, TSubclassOf<class AScavBullet> BulletClass, const FTransform& Transform, FVector BulletVelocity, AActor* BulletOwner, APawn* BulletInstigator);
	...
	float PenetrationTrace(FVector start, FVector end, TWeakObjectPtr<UPrimitiveComponent,FWeakObjectPtr> comp, EScavPenTraceType penType, TEnumAsByte<ECollisionChannel> channel, FVector &exitLoc, FVector &exitNormal);
